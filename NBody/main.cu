...
__global__ void MatMul(DATA_TYPE* matA, DATA_TYPE* matB, DATA_TYPE* matC, int m, int n, int k)
{
	...
	for (int i = 0; i < k; ++i)
		matC[cIndex] += __fmul_rn(matA[row * k + i], matB[i * n + col]);
}
...
int main1(int argc, char* argv[])
{
	...
	for (int i = 0; i < sizeA; i++) A[i] = ((rand() % 10) + ((DATA_TYPE)(rand() % 100) / 100.0));
	for (int i = 0; i < sizeB; i++) B[i] = ((rand() % 10) + ((DATA_TYPE)(rand() % 100) / 100.0));
	...
	******************************************************************/
	...
	cudaMalloc(&dA, sizeA * sizeof(DATA_TYPE)); cudaMemset(dA, 0, sizeA * sizeof(DATA_TYPE));
	cudaMalloc(&dB, sizeB * sizeof(DATA_TYPE)); cudaMemset(dB, 0, sizeB * sizeof(DATA_TYPE));
	cudaMalloc(&dC, sizeC * sizeof(DATA_TYPE)); cudaMemset(dC, 0, sizeC * sizeof(DATA_TYPE));
