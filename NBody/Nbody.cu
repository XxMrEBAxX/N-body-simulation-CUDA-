...
__host__ void CUDA_DataTransfer(std::vector<Body>& bodies)
{
...
__global__ void Integrate(double* dX, double* dY, double* dVelX, double* dVelY, double* dAccX, double* dAccY, double DT, int NUM_BODIES, int width, int height)
{
...
__global__ void ComputeForces(double* dX, double* dY, double* dVelX, double* dVelY, double* dAccX, double* dAccY, double* dMass, int NUM_BODIES, double EPS, double G)
{
	double forceX = 0;
	double forceY = 0;
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__host__ void CUDA_Running(std::vector<Body>& bodies, double delta, int width, int height)
{
	DS_timer timer(3);
	...
	timer.initTimer(0);
	timer.onTimer(0);
	ComputeForces <<< GRID_SIZE, BLOCK_SIZE >>> (d_posX, d_posY, d_velX, d_velY, d_accX, d_accY, d_mass, NUM_BODIES, EPS, G);
	...
	timer.initTimer(1);
	timer.onTimer(1);
	Integrate <<< GRID_SIZE, BLOCK_SIZE >>> (d_posX, d_posY, d_velX, d_velY, d_accX, d_accY, delta * DT, NUM_BODIES, width, height);
	cudaDeviceSynchronize();
	timer.offTimer(1);
